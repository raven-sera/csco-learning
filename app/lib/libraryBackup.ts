...
const FORMAT = 'csco-personal-library';
const VERSION = 1;
const CHUNK = 64 * 1024;
  ...
  const visit = (source: Node, parent: Node, depth: number) => {
    ...
    if (supportedTags[source.tagName] === true) {
      ...
      if (source.tagName === 'A') {
        ...
        if (/^https?:\/\//i.test(href) || /^mailto:[^\s<>]+$/i.test(href) || /^#csco-slide=[^\s<>]+$/.test(href)) {
...
function validateManifest(value: unknown, entries: Map<string, Entry>): Manifest {
  ...
  if (data.format !== FORMAT) fail('这不是 CSCO 个人资料库备份，请选择本工具导出的 ZIP。');
