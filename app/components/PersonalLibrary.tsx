...
const BROWSING_KEY = 'csco-personal-library-browsing-v1';
...
type BrowsingState = { query: string; type: ContentFilter; date: string; field: string; tag: string; sort: 'updated' | 'date'; expanded: boolean; scroll: number; visibleCount: number };
const DEFAULT_BROWSING: BrowsingState = { query: '', type: 'all', date: '', field: '', tag: '', sort: 'updated', expanded: false, scroll: 0, visibleCount: SHELF_PAGE_SIZE };
...
function loadBrowsing(): BrowsingState {
  try {
    ...
    return {
      ...
      visibleCount: typeof value.visibleCount === 'number' && Number.isSafeInteger(value.visibleCount) ? Math.max(SHELF_PAGE_SIZE, value.visibleCount) : SHELF_PAGE_SIZE,
    };
...
function readingPosition(item: LibrarySummary) {
  const section = validReadingSection(item, item.meta.reading?.section);
  if (section === 'slides') {
    const index = item.slides.findIndex((slide) => slide.id === item.meta.reading?.slideId);
    return `PPT · 第 ${Math.max(0, index) + 1} / ${item.slideCount} 页`;
  }
  if (section === 'text') return item.meta.reading?.textOffset ? '文字笔记 · 已保存上次位置' : '文字笔记 · 开篇';
  return `${item.recordingCount} 段录音 · 收听列表`;
}
...
function SlideThumbnail({ id, updatedAt }: { id: string; updatedAt: number }) {
  ...
  return <span className="libraryThumbnail" ref={container} aria-hidden="true">
    {source && !unavailable ? <img src={source} alt="" loading="lazy" onError={() => setUnavailable(true)} /> : <span>{unavailable ? 'PPT 预览不可用' : 'PPT'}</span>}
  </span>;
}

function BookCover({ item }: { item: LibrarySummary }) {
  const report = REPORT_BY_ID.get(item.reportId);
  return <span className={`libraryBookCover libraryBookTone${Math.abs(item.reportId) % 4}${item.firstSlideId ? ' libraryBookWithSlide' : ''}`} aria-hidden="true">
    <span className="libraryCoverField">{report?.field ?? '本机存档'}</span>
    <span className="libraryCoverTitle">{titleFor(item)}</span>
    {item.firstSlideId && <SlideThumbnail key={`${item.firstSlideId}:${item.updatedAt}`} id={item.firstSlideId} updatedAt={item.updatedAt} />}
    <span className="libraryCoverAuthor">{report?.speaker || (report ? report.institution : `报告 #${item.reportId}`)}</span>
    <span className="libraryCoverFoot">{item.slideCount ? `${item.slideCount} 页 PPT` : item.hasNote ? '文字笔记' : `${item.recordingCount} 段录音`}</span>
  </span>;
}
...
function LibraryContent({ onOpen }: { onOpen: (report: Report, options: NotebookOpenOptions) => void }) {
  ...
  const changeBrowsing = (patch: Partial<BrowsingState>) => {
    const resetShelf = patch.query !== undefined || patch.type !== undefined || patch.date !== undefined || patch.field !== undefined || patch.tag !== undefined || patch.sort !== undefined;
    const next = { ...browsingRef.current, ...(resetShelf ? { visibleCount: SHELF_PAGE_SIZE } : {}), ...patch };
  ...
  const open = async (item: LibrarySummary, options: NotebookOpenOptions, resume = false) => {
    ...
    if (resume) {
      ...
      try {
        ...
        const slideId = item.slides.some((slide) => slide.id === meta.reading?.slideId) ? meta.reading?.slideId : item.firstSlideId;
        target = { ...options, section, slideId: section === 'slides' ? slideId : undefined };
      } catch {
        if (mounted.current && current === openRequest.current) {
          setNotice('未能读取上次阅读位置。请重试，或展开笔记本“详情”后选择 PPT、文字或录音入口。');
          setOpening(null);
        }
        return;
      }
  ...
  const latest = useMemo(() => items.reduce<LibrarySummary | undefined>((result, item) => REPORT_BY_ID.has(item.reportId) && item.meta.lastOpenedAt > (result?.meta.lastOpenedAt ?? 0) ? item : result, undefined), [items]);
  ...
  const displayed = visible.slice(0, browsing.visibleCount);

  return <section className="personalLibrary" aria-labelledby="personal-library-title">
    <header className="libraryHeading">
      <div><span className="libraryEyebrow">MYCO · 我的书架</span><h1 id="personal-library-title">个人图书馆</h1><p>一场报告，一本笔记。把知识留在手边，随时接着读。</p></div>
    ...
    {loaded && <div className="libraryTotals" aria-label="资料库概况"><strong>{totals.all}<span>本笔记</span></strong><span>{totals.photos} 页 PPT</span><span>{totals.text} 份文字</span><span>{totals.recordings} 段录音</span></div>}
    {latest && <section className="libraryContinue" aria-label="继续上次阅读">
      <button type="button" className="libraryContinueCover" aria-label={`继续阅读 ${titleFor(latest)}`} disabled={opening === latest.reportId} onClick={() => void open(latest, { mode: 'read' }, true)}><BookCover item={latest} /></button>
      <div className="libraryContinueCopy"><span className="libraryEyebrow">继续阅读</span>
        <h2><button type="button" disabled={opening === latest.reportId} onClick={() => void open(latest, { mode: 'read' }, true)}>{titleFor(latest)}</button></h2>
        <p>{REPORT_BY_ID.get(latest.reportId)?.speaker} · {timestamp(latest.meta.lastOpenedAt)}</p>
        <span className="libraryReadingPosition">{readingPosition(latest)}</span>
      </div>
      <div className="libraryContinueActions"><button type="button" className="libraryPrimary" disabled={opening === latest.reportId} onClick={() => void open(latest, { mode: 'read' }, true)}>{opening === latest.reportId ? '正在打开…' : '接着读'}</button><button type="button" disabled={opening === latest.reportId} onClick={() => void open(latest, { mode: 'edit' }, true)}>编辑笔记</button></div>
    </section>}
    ...
    <ul className="libraryList" aria-label="笔记书架" aria-busy={loading}>{displayed.map((item) => {
      ...
      const tagHits = item.meta.tags.filter((tag) => tag === browsing.tag || tokens.some((token) => normalize(tag).includes(token)));
      ...
      const resume = !annotationHits.length && !textHit && browsing.type === 'all';
      return <li className="libraryCard" key={item.reportId}>
        <button type="button" className="libraryCoverButton" aria-label={`阅读 ${titleFor(item)}`} disabled={opening === item.reportId} onClick={() => void open(item, { ...target, mode: 'read' }, resume)}><BookCover item={item} /></button>
        <div className="libraryCardBody">
          <h3><button type="button" disabled={opening === item.reportId} onClick={() => void open(item, { ...target, mode: 'read' }, resume)}>{titleFor(item)}</button></h3>
          <p className="librarySpeaker">{report?.speaker || (report ? report.institution : `报告 #${item.reportId} · 未关联日程`)}</p>
          <div className="libraryCountBadges" aria-label="已保存内容">{item.slideCount > 0 && <span>{item.slideCount} 页 PPT</span>}{item.hasNote && <span>文字</span>}{item.recordingCount > 0 && <span>{item.recordingCount} 段录音</span>}</div>
          ...
          {textHit && <button type="button" className="libraryTextHit" onClick={() => void open(item, { section: 'text', mode: 'read' })}><b>命中文字笔记</b><span>{excerpt(item.noteText, tokens)}</span></button>}
          {tagHits.length > 0 && <div className="libraryTags" aria-label="匹配的个人标签">{tagHits.map((tag) => <button type="button" key={tag} aria-pressed={browsing.tag === tag} onClick={() => changeBrowsing({ tag: browsing.tag === tag ? '' : tag })}>#{tag}</button>)}</div>}
          {!report && <p className="libraryOrphan">原报告不在当前日程中，资料仍保留。请完整备份。</p>}
          <div className="libraryBookTools">
            <details className="libraryBookDetails"><summary aria-label={`${titleFor(item)} 的详情`}>详情</summary>
              <div className="libraryDetailsBody">
                <p className="libraryCardContext">{report?.field ?? '未关联日程'} · {report?.dateTime || '报告日期未知'}</p>
                {report && <p>{[report.institution, report.program, report.session, report.location].filter(Boolean).join(' · ')}</p>}
                <p className="libraryUpdated">更新于 {timestamp(item.updatedAt)}</p>
                {item.hasNote && <p className="libraryExcerpt">{excerpt(item.noteText, tokens) || '已保存文字笔记，打开查看完整内容。'}</p>}
                <div className="libraryContentCounts" aria-label="打开指定内容">{item.slideCount > 0 && <button type="button" onClick={() => void open(item, { section: 'slides', mode: 'read' })}>{item.slideCount} 页 PPT</button>}{item.hasNote && <button type="button" onClick={() => void open(item, { section: 'text', mode: 'read' })}>文字笔记</button>}{item.recordingCount > 0 && <button type="button" onClick={() => void open(item, { section: 'audio', mode: 'read' })}>{item.recordingCount} 段录音</button>}</div>
                ...
                {!report && <p>恢复到包含此报告的版本后可继续访问。完整 ZIP 备份会保留所有原始内容。</p>}
              </div>
            </details>
            {report ? <button type="button" className="libraryEditButton" aria-label={`编辑 ${titleFor(item)}`} disabled={opening === item.reportId} onClick={() => void open(item, { mode: 'edit' }, true)}>编辑</button> : <button type="button" className="libraryEditButton" onClick={() => setBackupOpen(true)}>备份</button>}
          </div>
        </div>
      </li>;
    })}</ul>
    {visible.length > displayed.length && <div className="libraryPagination"><p>已显示 {displayed.length} / {visible.length} 本</p><button type="button" onClick={() => changeBrowsing({ visibleCount: browsing.visibleCount + SHELF_PAGE_SIZE })}>再显示 {Math.min(SHELF_PAGE_SIZE, visible.length - displayed.length)} 本</button></div>}
    <p className="libraryBottomNote">资料只在本机保存。点击封面即可阅读；增补内容时选择“编辑”。</p>
