...
function getCalendarPrintRanges(reports:Report[]) {
  const intervals=reports.flatMap((report)=>{
    const match=report.dateTime.match(/^2026-09-(?:17|18|19).*?(\d{2}):(\d{2})-(\d{2}):(\d{2})/);
    if(!match)return [];
    return [{start:Number(match[1])*60+Number(match[2]),end:Number(match[3])*60+Number(match[4])}];
  });
  if(!intervals.length)return [{startMinute:8*60,endMinute:9*60}];
  const firstMinute=Math.max(8*60,Math.floor(Math.min(...intervals.map(({start})=>start))/60)*60);
  const lastMinute=Math.min(20*60,Math.max(
    firstMinute+60,
  ...
  return <div className="pdfDocument calendarDocument">
    {ranges.map(({startMinute,endMinute},pageIndex)=><section className="pdfPage calendarPdfPage" key={startMinute}>
      ...
      <header className="pdfCoverHeader">
        <div><span>THREE-DAY CALENDAR · CSCO 2026</span><h1>我的听会日历</h1></div>
...
export default function ExportCenter({
    ...
    const build=async()=>{
      try {
        ...
        const nextFilename=mode==='schedule'
          ? 'CSCO2026-私人听会日程-表格版.pdf'
          : mode==='calendar'
            ? 'CSCO2026-私人听会日程-日历版.pdf'
            : 'CSCO2026-听会笔记.pdf';
  ...
  return <><div className="exportOverlay" role="dialog" aria-modal="true" aria-labelledby="export-title"><div className={`exportModal ${stage==='preview'?'previewMode':''}`}>
    ...
    {stage==='schedule-style'&&<><p className="exportLead">已选 {reports.length} 场报告。请选择日程 PDF 的呈现形式。</p><div className="exportChoices scheduleStyleChoices"><button type="button" onClick={()=>choose('schedule')}><span>01</span><div><small>TABLE ITINERARY</small><h3>表格日程</h3><p>沿用原版清单，完整汇总题目、报告人、单位、地点、专场与参会打卡。</p></div><b>生成表格版 ↗</b></button><button type="button" onClick={()=>choose('calendar')}><span>02</span><div><small>CALENDAR ITINERARY</small><h3>日历日程</h3><p>仅显示 9.17–9.19；保留三天分列、时间纵轴与同时段并列，按完整日程分页排版，不受当前页面与日期筛选限制。</p></div><b>生成日历版 ↗</b></button></div>{clearAfterExport&&<p className="exportWarning">PDF 成功生成后，这 {reports.length} 条收藏将从灵动岛清空。</p>}</>}
    {stage==='generating'&&<div className="exportGenerating"><span className="generatingOrb">PDF</span><h3>正在排版你的私人文件</h3><p>{mode==='calendar'?'正在生成三天时间轴，并按小时分页保持文字可读。':'整理日程顺序、分页和中文字体，请稍候。'}</p></div>}
