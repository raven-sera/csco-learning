...
export default function ExportCenter({
  ...
  return <><div className="exportOverlay" role="dialog" aria-modal="true" aria-labelledby="export-title"><div className={`exportModal ${stage==='preview'?'previewMode':''}`}>
    ...
    {stage==='schedule-style'&&<><p className="exportLead">已选 {reports.length} 场报告。请选择日程 PDF 的呈现形式。</p><div className="exportChoices scheduleStyleChoices"><button type="button" onClick={()=>choose('schedule')}><span>01</span><div><small>TABLE ITINERARY</small><h3>表格日程</h3><p>沿用原版清单，完整汇总题目、报告人、单位、地点、专场与参会打卡。</p></div><b>生成表格版 ↗</b></button><button type="button" onClick={()=>choose('calendar')}><span>02</span><div><small>CALENDAR ITINERARY</small><h3>日历日程</h3><p>仅显示 9.17–9.19；三天分列、时间纵轴和同时段并列关系与网页日历保持一致。</p></div><b>生成日历版 ↗</b></button></div>{clearAfterExport&&<p className="exportWarning">PDF 成功生成后，这 {reports.length} 条收藏将从灵动岛清空。</p>}</>}
